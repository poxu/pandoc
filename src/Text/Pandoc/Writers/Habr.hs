...
-}
module Text.Pandoc.Writers.Habr ( writeHabr , writeHabrString ) where
...
writeHabrString :: WriterOptions -> Pandoc -> String
writeHabrString opts d =
...
writeHabr :: WriterOptions -> Pandoc -> Html
writeHabr opts d =
  let (body, context) = evalState (pandocToHtml opts d) defaultWriterState
  in  if writerStandalone opts
         then inTemplate opts context body
